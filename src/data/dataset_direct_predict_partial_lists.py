...
class GeneRegulationListDataset(Dataset):
    def __init__(self, csv_dir, split="train", tokenizer=None,
                 ...
                 list_format: str = "default",
                 test_split_cell_lines: str = "none",
                 partial_list_fraction: float = 0.0): 
        """
        ...
        Args:
            ...
            test_split_cell_lines (str, optional): Defines a cell line-based train/test
                                                   splitting strategy. See original docstring for details.
                                                   Defaults to "none".
            partial_list_fraction (float, optional): The fraction of the true up/down gene lists
                                                     to provide in the prompt, prompting the LLM
                                                     to guess the remainder. Must be between 0.0 and 1.0.
                                                     A value of 0.0 (default) means no genes are
                                                     provided, and the LLM must predict from scratch.
                                                     A value of 0.5 means 50% of the genes are
                                                     provided. Defaults to 0.0.
        """
        self.data = []
        ...
        if not 0.0 <= partial_list_fraction <= 1.0:
            raise ValueError(f"partial_list_fraction must be between 0.0 and 1.0, but got {partial_list_fraction}")
        self.partial_list_fraction = partial_list_fraction
        if self.partial_list_fraction > 0.0:
            logging.info(f"Partial list mode enabled. Providing {partial_list_fraction*100:.1f}% of genes in the prompt.")
    ...
    def __getitem__(self, idx):
        ...
        # The 'solution' ALWAYS contains the full, correct answer.
        # This part remains unchanged as it builds the ground truth target.
        if self.list_format == "default":
            ...
            text_solution_payload = f"UPREGULATED_GENES:\n{up_genes_list}\n\nDOWNREGULATED_GENES:\n{down_genes_list}"
            final_solution_text = f"<answer>\n{text_solution_payload}\n</answer>"
        
        # <<< SYSTEM PROMPT LOGIC >>>
        if self.prompt_mode == "default" and self.list_format == "default":
            system_prompt_string = (
                ...
                "as a result of this CRISPRi knockdown.\n\n"
        ...
        if self.prompt_mode == "default" and self.list_format == "bullet_list":
            system_prompt_string = (
                "You are an molecular and cellular biology expert analyzing and predicting gene regulation upon CRISPRi knockdown. "
                "your task is to list all genes that are upregulated and all genes that are downregulated "
                "as a result of this CRISPRi knockdown.\n\n"
        ...
        elif self.prompt_mode == "synth_data_with_ans" and self.list_format == "bullet_list":
            system_prompt_string = (
                "You are an molecular and cellular biology expert analyzing and predicting gene regulation upon CRISPRi knockdown. "
                f"The regulatory effect of knocking down the {item['pert']} FOR EVERY SINGLE GENE INDIVIDUALLY which is given to you: {text_solution_payload}.\n"
                ...
                "When answering provide your reasoning in regulatory effect such that you use the following template: "
                f"<think>[Your reasoning here]</think>"
                "<answer></answer> "
                ...
                "\n- Gene2"
                "\n</answer>"
                "\n\nInclude your reasoning before the answer using the <think> </think> tags:"
                "\n<think>"
                "\n[Your detailed reasoning about pathways, interactions, and mechanisms goes here.]"
                "\n</think>"
                "\n<answer>"
                "\n[Your formatted gene lists as described above]"
                "\n</answer>"
                "\n\nIMPORTANT: Follow this EXACT format with consistent capitalization, dashes for bullets, and spacing as shown."
            )
        elif self.prompt_mode == "synth_data_with_ans" and self.list_format == "default":
            system_prompt_string = (
                "You are an molecular and cellular biology expert analyzing and predicting gene regulation upon CRISPRi knockdown. "
        ...
        # <<< USER PROMPT LOGIC >>>
        if self.partial_list_fraction > 0.0:
            # --- Create partial lists to show in the prompt ---
            full_up_genes = item['upregulated_genes']
            full_down_genes = item['downregulated_genes']

            # Calculate how many genes to sample for each list
            num_to_provide_up = int(len(full_up_genes) * self.partial_list_fraction)
            num_to_provide_down = int(len(full_down_genes) * self.partial_list_fraction)

            # Randomly sample from the full lists and sort for consistent presentation
            provided_up_genes = sorted(random.sample(full_up_genes, num_to_provide_up))
            provided_down_genes = sorted(random.sample(full_down_genes, num_to_provide_down))

            # --- Format the partial lists for inclusion in the prompt ---
            if self.list_format == "default":
                # Matches the Python list __str__ format, e.g., ['GENE_A', 'GENE_B']
                up_genes_prompt_str = f"{provided_up_genes}"
                down_genes_prompt_str = f"{provided_down_genes}"
                prompt_intro = (
                    f"For the CRISPRi knockdown of gene {item['pert']} in {item['cell_type']} cells, "
                    f"I have identified a partial list of regulated genes.\n\n"
                    f"KNOWN UPREGULATED Genes: {up_genes_prompt_str}\n"
                    f"KNOWN DOWNREGULATED Genes: {down_genes_prompt_str}\n\n"
                    f"Based on this information, complete the lists and provide the full set of all "
                    f"upregulated and downregulated genes according to the specified format."
                )

            elif self.list_format == "bullet_list":
                # Matches the bulleted list format
                up_genes_prompt_str = "\n".join([f"- {gene}" for gene in provided_up_genes]) if provided_up_genes else "NONE"
                down_genes_prompt_str = "\n".join([f"- {gene}" for gene in provided_down_genes]) if provided_down_genes else "NONE"
                prompt_intro = (
                    f"For the CRISPRi knockdown of gene {item['pert']} in {item['cell_type']} cells, "
                    f"I have identified a partial list of regulated genes, shown below.\n\n"
                    f"KNOWN UPREGULATED genes:\n{up_genes_prompt_str}\n\n"
                    f"KNOWN DOWNREGULATED genes:\n{down_genes_prompt_str}\n\n"
                    f"Based on this, complete the analysis and provide the final, complete lists of ALL "
                    f"upregulated and downregulated genes in the specified format. NOTE: The nature of differential expression preprocessing caps the total number of regulated genes at no more than 20."
                )
            
            user_prompt_string = prompt_intro
        else:
            user_prompt_string = (
                f"For the CRISPRi knockdown of gene {item['pert']} in {item['cell_type']} cells, "
