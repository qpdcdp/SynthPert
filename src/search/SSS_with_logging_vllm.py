...
verification_prompts_dirct_prediction = [
    ...
    # Turn 6: Final Judgment on the candidate solution's gene list
...
def run_generate_vllm(llm_engine, prompts, temperature, max_new_tokens, num_return_sequences=1):
    """Runs model generation using the vLLM engine for a batch of prompts."""
    ...
    try:
        # vLLM's generate method takes a list of prompt strings directly
        outputs = llm_engine.generate(prompts, sampling_params, use_tqdm=False)

        # Process and return the results in a format your script expects
        all_generated_texts = []
        for output in outputs:
            prompt_completions = [completion.text.strip() for completion in output.outputs]
            all_generated_texts.append(prompt_completions)

        if len(prompts) == 1:
            return all_generated_texts[0] if num_return_sequences > 1 else all_generated_texts[0][0]
        else:
            # For batch processing, return the first completion for each prompt
            return [texts[0] for texts in all_generated_texts]
...
# --- Core Comparison Function (BATCHED for vLLM) ---
def batch_compare_all_pairs(
    llm_engine,
    question_prompt,
    valid_candidates,
    pairs_to_compare, # List of (cand_i_idx, cand_j_idx) tuples
    ktie,
    temperature,
    max_new_tokens
):
    """
    Performs multi-turn comparison for ALL pairs in parallel, turn-by-turn.
    Returns a dictionary mapping candidate index to their total number of matchup wins.
    """
    num_pairs = len(pairs_to_compare)
    if num_pairs == 0:
        return defaultdict(int)

    # Total parallel conversations = num_pairs * ktie
    histories = [""] * (num_pairs * ktie)

    initial_contexts = []
    for idx_i, idx_j in pairs_to_compare:
        s1_text = valid_candidates[idx_i]
        s2_text = valid_candidates[idx_j]
        for _ in range(ktie):
    ...
    # Loop through the TURNS of the comparison conversation
    responses = None
    for turn_idx, prompt_template in enumerate(comparison_prompts):
        batch_prompts = []
        for i in range(num_pairs * ktie):
            context = initial_contexts[i].copy()
            if turn_idx > 0:
                context["history"] = histories[i]
            batch_prompts.append(prompt_template.format(**context))

        print(f"  Comparing Turn {turn_idx+1}/{len(comparison_prompts)} for {num_pairs} pairs ({len(batch_prompts)} total prompts)...")
        responses = run_generate_vllm(llm_engine, batch_prompts, temperature, max_new_tokens)

        if not responses or len(responses) != len(histories):
            print("Error: Mismatch in response count during comparison. Aborting.")
            return defaultdict(int)

        for i in range(len(histories)):
            histories[i] += f"--- Turn {turn_idx+1} ---\n{responses[i]}\n\n"

    # After all turns, parse the final judgments to determine matchup winners
    final_judgment_responses = responses
    matchup_outcomes = defaultdict(int) # This will store wins for each candidate

    # Iterate through the matchups
    for pair_idx, (cand_i_idx, cand_j_idx) in enumerate(pairs_to_compare):
        i_wins_count = 0
        # Count wins for candidate 'i' within its 'ktie' attempts for this specific pair
        for k in range(ktie):
            response_idx = (pair_idx * ktie) + k
            final_response = final_judgment_responses[response_idx]
            # Find the last digit '1' or '2' in the response string
            matches = re.findall(r'\b([12])\b', final_response)
            if matches:
                if matches[-1] == '1': # '1' corresponds to s1, which is candidate i
                    i_wins_count += 1
            else:
                 print(f"Warning: Could not parse comparison judgment: '{final_response}'. No win awarded for this attempt.")

        j_wins_count = ktie - i_wins_count

        # Award one "matchup win" to the overall winner of this pair
        if i_wins_count > j_wins_count:
            matchup_outcomes[cand_i_idx] += 1
        elif j_wins_count > i_wins_count:
            matchup_outcomes[cand_j_idx] += 1
        # If it's a tie (e.g., ktie=2, 1-1 wins), no matchup win is awarded.

    return matchup_outcomes

# --- NEW: Core Verification Function (Mega-Batched) ---
def batch_verify_all_candidates(llm_engine, tokenizer, question_prompt, candidates, kverif, temperature, max_new_tokens, task, pert, cell_type):
    """
    Performs multi-turn verification for ALL candidates in parallel, turn-by-turn.
    This is much faster than verifying one candidate at a time.
    """
    num_candidates = len(candidates)
    if num_candidates == 0:
        return {}

    verification_prompts = verification_prompts_single_gene_prediction if task == "single_gene_prediction" else verification_prompts_dirct_prediction
    
    # We need to track kverif independent conversations for EACH candidate.
    # Total parallel conversations = num_candidates * kverif
    # We use a flat list and indices to manage this.
    histories = [""] * (num_candidates * kverif)
    
    # Create the initial context for every single verification attempt
    initial_contexts = []
    for i, candidate_response in enumerate(candidates):
        for _ in range(kverif):
            initial_contexts.append({
                "question": question_prompt, 
    ...
    # Loop through the TURNS of the verification conversation
    for turn_idx, prompt_template in enumerate(verification_prompts):
        # 1. Prepare a single, massive batch of prompts for the current turn
        batch_prompts = []
        for i in range(num_candidates * kverif):
            context = initial_contexts[i].copy() # Start with base info
            if turn_idx > 0:
                context["history"] = histories[i] # Add specific history for this attempt
            
            try:
                formatted_prompt = prompt_template.format(**context)
                batch_prompts.append(formatted_prompt)
            except KeyError as e:
                print(f"Error formatting verification prompt: Missing key {e}")
                batch_prompts.append("FORMATTING_ERROR") # Add placeholder

        # 2. Run the ENTIRE BATCH through vLLM in ONE call
        print(f"  Verifying Turn {turn_idx+1}/{len(verification_prompts)} for {num_candidates} candidates ({len(batch_prompts)} total prompts)...")
        responses = run_generate_vllm(llm_engine, batch_prompts, temperature, max_new_tokens)

        # 3. Update the history for each parallel conversation
        if responses and len(responses) == len(histories):
            for i in range(len(histories)):
                histories[i] += f"--- Turn {turn_idx+1} ---\n{responses[i]}\n\n"
        else:
            print("Error: Mismatch in response count during verification. Aborting.")
            # Handle error case, maybe return empty results
            return {}

    # After all turns, parse the final responses
    final_responses = responses
    verification_results = defaultdict(lambda: {"scores": [], "details": []})
    
    for i in range(num_candidates * kverif):
        candidate_idx = i // kverif
        final_response_text = final_responses[i]
        
        parsed_score = 0.0
        if task == "single_gene_prediction":
            if "correct" in final_response_text.lower():
                parsed_score = 1.0
        elif task == "direct_prediction":
            match = re.search(r'Upregulated:\s*(\d+).*?Downregulated:\s*(\d+)', final_response_text, re.DOTALL)
            if match:
                parsed_score = (int(match.group(1)) + int(match.group(2))) / 200.0
        
        verification_results[candidate_idx]["scores"].append(parsed_score)
        verification_results[candidate_idx]["details"].append({
    ...
    final_scores = {}
    for idx, data in verification_results.items():
        avg_score = sum(data["scores"]) / len(data["scores"]) if data["scores"] else 0.0
        final_scores[idx] = {
            "average_score": avg_score,
            "attempts": data["details"]
        }
        
    return final_scores

# --- REVISED: Core Search Function with Mega-Batching ---
def sampling_based_search_with_detailed_logging(
    ...
    """
    Runs the full search process using mega-batching for verification and tie-breaking,
    returning a comprehensive log dictionary and printing live analysis.
    """
    # Helper function print_candidate_summary_table remains the same...
    def print_candidate_summary_table(candidates, verification_scores, ground_truth, task):
        ...
        for i, verif_rank_item in enumerate(sorted_by_verif):
            ...
            line = (
                f"{i+1:<16} | {gt_rank:<14} | {cand_index:<12} | "
                f"{verif_rank_item['verif_score']:<14.3f} | {verif_rank_item['gt_score']:<20.3f}"
            )
            print(line)
        ...
        if sorted_by_gt:
            ...
            print(f"True Best (highest GT score):       Candidate {best_gt_candidate_idx}")
            
            if sorted_by_verif and best_verif_candidate_idx == best_gt_candidate_idx:
                print(">> RANKING SUCCESS: Model correctly identified the best candidate.")
            else:
                print(">> RANKING MISMATCH: Model did NOT identify the best candidate.")

    # Initialize the log object...
    item_log = {
        "item_details": {
            "pert": dataset_item.get("pert"), "cell_type": dataset_item.get("cell_type"),
        ...
        "generations": [], "verifications": {}, "tie_break": None, "final_result": {}
    }
    ...
    # --- Stage 1: Generation ---
    print(f"--- Stage 1: Generating {kinf} Candidates ---")
    candidate_responses = run_generate_vllm(
        llm_engine, question_prompt, sigma_inf,
        max_new_tokens_generate, num_return_sequences=kinf
    )
    valid_candidates = [r for r in candidate_responses if r != "GENERATION_ERROR"]
    for i, response in enumerate(valid_candidates):
        item_log["generations"].append({"candidate_index": i, "response_text": response})
    ...
    all_verification_results = batch_verify_all_candidates(
        llm_engine, tokenizer, question_prompt, valid_candidates, kverif,
        sigma_verif, max_new_tokens_verify, task,
        dataset_item.get("pert"), dataset_item.get("cell_type")
    )
    verification_scores = {}
    for i in range(len(valid_candidates)):
        result = all_verification_results.get(i)
        if result:
            verification_scores[i] = result["average_score"]
            item_log["verifications"][f"candidate_{i}"] = result
        else:
            verification_scores[i] = 0.0
            item_log["verifications"][f"candidate_{i}"] = {"average_score": 0.0, "attempts": []}

    # Live console analysis...
    ground_truth_label = item_log["item_details"]["ground_truth_label"]
    print_candidate_summary_table(
        candidates=valid_candidates, verification_scores=verification_scores,
        ground_truth=ground_truth_label, task=task
    )
    ...
    print(f"Found {len(SBest_indices)} candidates for tie-breaking. Indices: {SBest_indices}")
    ...
    if len(SBest_indices) == 1:
        ...
        item_log["tie_break"] = {"candidates_in_tie_break": SBest_indices}

        pairs_to_compare = [(SBest_indices[i], SBest_indices[j]) for i in range(len(SBest_indices)) for j in range(i + 1, len(SBest_indices))]

        # Run the corrected comparison function
        matchup_wins = batch_compare_all_pairs(
            llm_engine,
            question_prompt,
            valid_candidates, # Pass the list of candidate strings
            pairs_to_compare,
            ktie,
            sigma_tie,
            max_new_tokens_compare
        )
        item_log["tie_break"]["matchup_total_wins"] = dict(matchup_wins) # Log the wins
        print(f"Matchup win counts: {dict(matchup_wins)}")

        # Determine the winner based on total matchup wins.
        # This handles cases where some candidates in SBest might not have any wins.
        candidates_with_wins = [idx for idx in SBest_indices if idx in matchup_wins]

        if not candidates_with_wins:
            print("Tie-breaking resulted in no definitive matchup wins. Resolving by highest verification score.")
            # Fall back to the highest verification score among those in the tie-break
            best_index = max(SBest_indices, key=lambda k: verification_scores[k])
        else:
            max_wins = max(matchup_wins.values())
            # Find all candidates who achieved the maximum number of wins
            top_winners = [idx for idx, wins in matchup_wins.items() if wins == max_wins]

            if len(top_winners) > 1:
                print(f"Tie-breaking resulted in a tie in wins ({max_wins} wins) between candidates {top_winners}. Resolving by verification score.")
                # If there's a tie in wins, use the verification score to break *that* tie
                best_index = max(top_winners, key=lambda k: verification_scores[k])
            else:
                # A single clear winner from the tournament
                best_index = top_winners[0]

        item_log["tie_break"]["final_winner_index"] = best_index
        print(f"Tie-breaking complete. Final winner: Candidate {best_index}")

# --- Main Evaluation Function ---
def evaluate_dataset_with_search(csv_dir, model_name, output_dir, llm_engine, tokenizer, dataset_fraction, task="single_gene_prediction", cell_type_split="default", list_format="bullet_list", **search_params):
    ...
    if dataset_fraction < 1.0:
        # If dataset_fraction is less than 1.0, sample a random fraction of the dataset
        total_samples = len(test_dataset)
        sample_size = int(total_samples * args.dataset_fraction)
        indices = random.sample(range(total_samples), sample_size)
        test_dataset = torch.utils.data.Subset(test_dataset, indices)
        print(f"Loaded {len(test_dataset)} samples from the dataset for evaluation.")
    else:
        print(f"Loaded full dataset with {len(test_dataset)} samples for evaluation.")
    ...
    for idx in tqdm(range(len(test_dataset)), desc="Evaluating Samples"):
        ...
        print(f"===== Finished Item {idx} =====")


    final_logs = all_item_logs
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_fraction", type=float, default=1.0, help="Fraction of dataset to use for evaluation")
    ...
    llm_engine = LLM(
        ...
        # Optional: increase gpu_memory_utilization if you have a lot of VRAM
