...
def accuracy_reward_fn(completion: str,
    ...
    # Define rewards for each class
    class_rewards = {
        "upregulated": 10.0,
        "downregulated": 10.0,
        "not differentially expressed": 0.5
    }
    ...
    try:
        ...
        if extracted_answer is not None:
            ...
            if generated_answer_normalized == ground_truth_answer:
                # Return the reward specific to the correct class
                reward = class_rewards.get(ground_truth_answer, 1.0) 
            else:
                # Penalize any incorrect prediction, maybe based on the missed class
                reward = -5.0 # A significant penalty for being wrong
        else:
            reward = -10.0 # Penalty for missing tag
